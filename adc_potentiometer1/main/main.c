// การทดลองอ่านค่า LDR ด้วย ADC (ESP-IDF)
// บอร์ด: ESP32, ช่อง ADC: ADC1_CHANNEL_7 (GPIO35)
...
void app_main(void)
{
    ...
    // หมายเหตุ: บน ESP-IDF v5.5 ควรใช้ ADC_ATTEN_DB_12 (ADC_ATTEN_DB_11 ถูก deprecate แต่พฤติกรรมเทียบเท่า)
    ...
    while (1) {
        ...
        // แปลงเป็นระดับแสง (สมมติเป็นสัดส่วนตรง)
        float lightLevel = (adc_reading / 4095.0f) * 100.0f;

        // กำหนดสถานะแสงแบบหยาบ
        ...
        // แสดงผลลัพธ์
        ESP_LOGI(TAG, "ADC: %4d | แรงดัน: %.2f V | ระดับแสง: %.1f%% | สถานะ: %s",
